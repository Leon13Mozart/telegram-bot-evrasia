...
def init_mk_schedule_schema():
    ...
    with sqlite3.connect(DATABASE) as conn:
        ...
        # Ліміти місць на майстер-класи. Усі значення зберігаються локально в SQLite.
        conn.execute(
            """
...
async def _complete_mk_registration_with_schedule(
    *, context, group_id, group_title, user_id, requester_name,
    ...
    with sqlite3.connect(DATABASE, timeout=10) as conn:
        ...
        try:
            _mk_assert_capacity(conn, group_id, event_date, infer_children_count(child_name))
        except ValueError as exc:
            conn.rollback()
            await context.bot.send_message(chat_id=group_id, text=f"⚠️ {exc}")
            return None
        cursor = conn.execute(
            """
            INSERT INTO mk_registrations(
...
def _external_key(value):
    ...
    # У Google Form адреса може містити латинську e у номері (27e),
    # тоді як у Telegram/SQLite — кириличну е (27е).
    value = re.sub(r"(?<=\d)e\b", "е", value)
...
# ============================================================
# ЛІМІТИ МІСЦЬ НА МАЙСТЕР-КЛАСИ
# ============================================================

def _mk_limit_state(conn, group_id: int, event_date: str):
    """Повертає (enabled, seat_limit, booked_children, remaining)."""
    row = conn.execute(
    ...
    row = conn.execute(
        "SELECT seat_limit FROM mk_date_limits WHERE group_id=? AND event_date=?",
        (group_id, event_date),
    ).fetchone()
    seat_limit = None if row is None else int(row[0])

    names = conn.execute(
        "SELECT child_name FROM mk_registrations WHERE group_id=? AND event_date=? AND COALESCE(attendance_status,'yes') <> 'no'",
        (group_id, event_date),
    ).fetchall()
    booked = sum(infer_children_count(r[0] or "") for r in names)
    remaining = None if seat_limit is None else max(0, seat_limit - booked)
    return enabled, seat_limit, booked, remaining


def _mk_assert_capacity(conn, group_id: int, event_date: str, requested_children: int):
    enabled, seat_limit, booked, remaining = _mk_limit_state(conn, group_id, event_date)
    if not enabled:
        raise ValueError("Запис на майстер-класи для цього ресторану вимкнено")
    if seat_limit is not None and booked + max(1, requested_children) > seat_limit:
        if remaining <= 0:
            raise ValueError("На цю дату вільних місць уже немає")
        raise ValueError(f"На цю дату залишилось лише {remaining} місць")
    return enabled, seat_limit, booked, remaining
...
async def web_google_form_registration(request: Request):
    ...
    with sqlite3.connect(DATABASE, timeout=10) as conn:
        ...
        try:
            _mk_assert_capacity(conn, group_id, event_date, children_count)
        except ValueError as exc:
...
def web_limits(request: Request):
    current = _web_current_user(request)
    if current["role"] not in {ROLE_DEVELOPER, ROLE_MANAGER}:
        raise HTTPException(status_code=403, detail="Немає доступу до налаштування лімітів")
    ...
    with _web_db() as conn:
        date_rows = conn.execute(
            ...
            SELECT DISTINCT event_date FROM (
                SELECT event_date FROM mk_schedule WHERE group_id IN ({placeholders})
                UNION
                SELECT event_date FROM mk_registrations WHERE group_id IN ({placeholders})
                UNION
                SELECT event_date FROM mk_date_limits WHERE group_id IN ({placeholders})
            )
            WHERE event_date IS NOT NULL AND event_date <> '' AND event_date >= ?
            ORDER BY event_date
            """,
            [*allowed, *allowed, *allowed, today],
        ).fetchall()
        ...
        for group_id, title in current["groups"]:
            ...
            for event_date in dates:
                _enabled, seat_limit, booked, remaining = _mk_limit_state(conn, group_id, event_date)
                limits[event_date] = seat_limit
                occupancy[event_date] = {"booked": booked, "remaining": remaining}
            restaurants.append({
                "id": group_id,
                "name": title,
                "enabled": enabled,
    ...
    return {"dates": dates, "restaurants": restaurants}
...
async def web_save_limits(request: Request):
    ...
    if not isinstance(restaurants, list):
        raise HTTPException(status_code=400, detail="Поле restaurants має бути списком")
    ...
    with _web_db() as conn:
        ...
        for item in restaurants:
            ...
            if not isinstance(limits, dict):
                raise HTTPException(status_code=400, detail="Некоректні ліміти ресторану")
            for event_date, raw_limit in limits.items():
                try:
                    datetime.strptime(event_date, "%Y-%m-%d")
                except Exception:
                    raise HTTPException(status_code=400, detail=f"Некоректна дата: {event_date}")
                if raw_limit is None or raw_limit == "":
                    ...
                    continue
                try:
                    seat_limit = int(raw_limit)
                except Exception:
                    raise HTTPException(status_code=400, detail="Ліміт має бути цілим числом")
                if seat_limit < 0 or seat_limit > 999:
                    raise HTTPException(status_code=400, detail="Ліміт має бути від 0 до 999")
                conn.execute(
                    """
                    INSERT INTO mk_date_limits(group_id, event_date, seat_limit, updated_at)
                    VALUES(?,?,?,?)
                    ON CONFLICT(group_id,event_date) DO UPDATE SET seat_limit=excluded.seat_limit, updated_at=excluded.updated_at
                    """,
                    (group_id, event_date, seat_limit, now),
                )
        conn.commit()
    return {"ok": True}
